...
mod multisig_wallet {
    ...
    impl MultisigWallet {
        ...
        #[ink(message)]
        pub fn add_signatory(&mut self, signatory: AccountId) -> Result<(), &'static str> {
            if self.signatories.contains_key(&signatory) {
                return Err("Signatory already exists");
            }
            self.signatories.insert(signatory, true);
            Ok(())
        }
    ...
    mod tests {
        ...
        #[ink::test]
        fn test_add_signatory() {
            let accounts = ink_env::test::default_accounts::<ink_env::DefaultEnvironment>();
            let mut wallet = MultisigWallet::new(vec![accounts.david], 1);

            assert!(wallet.is_signatory(accounts.david));
            assert!(!wallet.is_signatory(accounts.isaac));

            assert_eq!(wallet.add_signatory(accounts.isaac), Ok(()));
            assert!(wallet.is_signatory(accounts.isaac));
        }
